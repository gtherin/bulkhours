...
class Boid:
    ...
    def __init__(self, position=None, vitesse=None, color=None) -> None:
        self.x = position if position is not None else np.random.uniform(-Boid.taille, Boid.taille, 2)
    ...
    def voisins(self, population: "Iterable[Boid]", seuil: float) -> "list[Boid]":
        ...
        return sorted(
            (
                ...
                if self is not other and not self.angle_mort(other) and self.distance(other) < seuil
            ),
            key=self.distance,
        )
    ...
    def separation(self, population: "Iterable[Boid]"):
        ...
        return sum(self.x - other.x for other in self.voisins(population, 50)[: Boid.max_voisins])
...
class Simulation:
    ...
    def plot(self, ax) -> None:
        ...
        for boid in self.boids:
            p, *_ = ax.plot(*boid.x, color=boid.color, markersize=15, marker=rotate_marker(boid_shape, boid.direction))
