...
def download_kaggle_data(filename, chunck_size=40960):
    ...
    with urlopen(download_url) as fileres, NamedTemporaryFile() as tfile:
        total_length = int(float(fileres.headers['content-length'])*1e-6)
        print(f'\033[36mDownloading {destination_path}{filename} data ({total_length} MO compressed)\033[39m')
        dl = 0
        ...
        while len(data) > 0:
            ...
            sys.stdout.write(f"\033[36m\r[{'=' * done}{' ' * (50-done)}] {dl} MO downloaded\033[39m")
            sys.stdout.flush()
            data = fileres.read(chunck_size)
