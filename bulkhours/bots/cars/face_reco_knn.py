...
def predict(X_img_path, knn_clf=None, model_path=None, distance_threshold=0.6):
    ...
    if not os.path.isfile(X_img_path) or os.path.splitext(X_img_path)[1][1:] not in {"png", "jpg", "JPG", "jpeg"}:
...
if __name__ == "__main__":
    # Train image
    # train("bulkhours/bots/images/data", model_save_path="bulkhours/bots/images/trained_knn_model.clf", n_neighbors=2) "knn_examples/train"

    # Using the trained classifier, make predictions for unknown images
    tdir = "bulkhours/bots/images/data/Sylvia"
    for image_file in os.listdir(tdir):
        full_file_path = os.path.join(tdir, image_file)

        print(f"Looking for faces in {full_file_path}")

        # Find all people in the image using a trained classifier model
        # Note: You can pass in either a classifier file name or a classifier model instance
        predictions = predict(full_file_path, model_path="bulkhours/bots/images/trained_knn_model.clf")

        # Print results on the console
        for name, (top, right, bottom, left) in predictions:
            print("- Found {name} at ({left}, {top})")
